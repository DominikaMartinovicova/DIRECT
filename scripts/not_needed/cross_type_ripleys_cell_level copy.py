...
#coi_list = args.coi
coi_list=["B_cell", "T_cell_regulatory","Macrophage_alveolar","Tumor_cells"]
...
#with open(args.output + f'{core}_ripleys_results.pkl', 'wb') as f:
...
print(f'Z-scores shape: {z_scores.shape}, z_scores[:5,:5,0]: \n {z_scores[:5,:5,0]}, nans: {np.sum(np.isnan(z_scores))}')

# filter low support simulations and invalid values
min_valid = int(args.n_sim * 0.2)   # at least 20% of simulations must be valid to trust the result for that cell-interaction pair

invalid_mask = ((sim_valid == 0)[..., None] |np.isnan(obs_curve) |(sim_std <= 1e-2)) # |(sim_valid < min_valid)[..., None])
...
print(f'after invalid mask Z-scores shape: {z_scores.shape}, z_scores[:5,:5,0]: \n {z_scores[:5,:,0]}, nans: {np.sum(np.isnan(z_scores))}')

print(f'Sum column-wise, interactions x radii: \n {np.sum(z_scores, axis=1)} \n non-nan count column-wise, interactions x radii: \n {np.sum(~np.isnan(z_scores), axis=1)}')
